...
standardise<-function(mean,normal,sd) {
  ...
  mean$data[]<-(mean$data-normal$data)/sd$data
  # convert from normal to uniform distribution
  mean$data[]<-0.5*erfc((-1*mean$data)/sqrt(2))
  return(mean)
}
...
cduvb<-fill.var('cduvb',grid,365,'mean')
cduvb.std<-cduvb
mx<-max(as.vector(cduvb$data))
cduvb.std$data[]<-cduvb.std$data/mx
