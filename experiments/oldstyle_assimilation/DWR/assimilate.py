...
# Estimate prmsl fields in the vicinity of the UK from (simulated)
#  observations at the locations of the DWR stations.
...
# We're going to look just at data close to the UK. 
# So rearange the data to be on -180 to 180 lon rather than 0-360
#  and discard all the points far from the UK.
# Needs its dimensions divisible by 8 for the convolutions
def to_UK_region(ict):
   ict=tf.reshape(ict,[91,180])
   regrid=tf.concat((ict[:,90:180],
                     ict[:,0:90]),1)
   uk=regrid[62:86,66:114]
   uk=tf.reshape(uk,[24,48,1])
   return uk

field_data = field_data.map(to_UK_region)
...
field_test_data = field_test_data.map(to_UK_region)
