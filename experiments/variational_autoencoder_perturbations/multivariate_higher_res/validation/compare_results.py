...
def rr_cube(cbe):
    ...
    # Latitudes cover -90 to 90 with 159 values
    lat_values=numpy.arange(-90,91,180/158)
    latitude = iris.coords.DimCoord(lat_values,
                                    standard_name='latitude',
                                    units='degrees_north',
                                    coord_system=cs)
    # Longitudes cover -180 to 180 with 319 values
    lon_values=numpy.arange(-180,181,360/318)
...
prmsl_t = tf.reshape(prmsl_t,[159,319,1])
t2m_t = t2m.copy()
t2m_t.data -= 280
t2m_t.data /=  50
t2m_t = tf.convert_to_tensor(t2m_t.data, numpy.float32)
t2m_t = tf.reshape(t2m_t,[159,319,1])
z500_t = z500.copy()
z500_t.data -= 5300
z500_t.data /= 600
z500_t = tf.convert_to_tensor(z500_t.data, numpy.float32)
z500_t = tf.reshape(z500_t,[159,319,1])
ict = tf.concat([prmsl_t,t2m_t,z500_t],2) # Now [159,319,3]
ict = tf.reshape(ict,[1,159,319,3])
result = autoencoder.predict_on_batch(ict)
result = tf.reshape(result,[159,319,3])
prmsl_r = prmsl.copy()
prmsl_r.data = tf.reshape(result.numpy()[:,:,0],[159,319]).numpy()
prmsl_u=prmsl_r.copy()
prmsl_r.data *= 3000
prmsl_r.data += 101325
t2m_r = t2m.copy()
t2m_r.data = tf.reshape(result.numpy()[:,:,1],[159,319]).numpy()
t2m_u=t2m_r.copy()
t2m_r.data *= 50
t2m_r.data += 280
z500_r = z500.copy()
z500_r.data = tf.reshape(result.numpy()[:,:,2],[159,319]).numpy()
z500_u=z500_r.copy()
z500_r.data *= 600
z500_r.data += 5300
