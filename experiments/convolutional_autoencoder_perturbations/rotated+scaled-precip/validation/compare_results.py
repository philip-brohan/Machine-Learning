...
# Get the 20CR data
ic=twcr.load('prate',datetime.datetime(2010,3,12,18),
                           version='2c')
...
if aspect<1:
    ...
    ax.set_ylim((dmean-(dmean-dmin)*aspect),
                (dmean+(dmax-dmean)*aspect))
else:
    ax.set_ylim(dmin,dmax)
    ax.set_xlim((dmean-(dmean-dmin)*aspect),
                (dmean+(dmax-dmean)*aspect))
