...
# Show the autoencoder weights compared with the original version
...
def axes_geom(layer=0,channel=0,nchannels=36,side='left'):
     if side=='left':
         if layer==0: 
...
def plot_hidden(weights,side='left'):
     # Single axes - var v. time
     if side=='left':
         ax=fig.add_axes([0.05,0.425,0.425,0.15])
     else:
         ax=fig.add_axes([0.55,0.425,0.425,0.15])
...
#  for original and perturbed autoencoders
fig=Figure(figsize=(20,12),  # Width, Height (inches)
           dpi=100,
...
# Hidden layer
plot_hidden(autoencoder.get_weights()[1],side='left')
...
for layer in [0,2]:
    ...
    for channel in order:
        ...
        plot_weights(w_in,layer=layer,channel=count,nchannels=36,
                     vmin=vmin,vmax=vmax,side='left')
        count += 1
        
# Get the pereturbed autoencoder
model_save_file=(("%s/Machine-Learning-experiments/"+
                  "simple_autoencoder_activations/softmax/"+
                 "saved_models/Epoch_%04d")) % (
                     os.getenv('SCRATCH'),100)
autoencoder=tf.keras.models.load_model(model_save_file)
...
# Hidden layer
plot_hidden(autoencoder.get_weights()[1],side='right')
...
for layer in [0,2]:
    ...
    for channel in order:
        ...
        w_in.data *= numpy.sign(autoencoder.get_weights()[1][channel])
        plot_weights(w_in,layer=layer,channel=count,nchannels=36,
                     vmin=vmin,vmax=vmax,side='right')
        count += 1
