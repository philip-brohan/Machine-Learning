...
prediction=[]
indices = ['random','annual','diurnal',args.station]
#indices = ['random','annual','diurnal']
for i in range(model_meta['source_len'], 
               len(obs[args.station])-model_meta['target_len']):
    source=numpy.reshape(obs[indices].values[i-model_meta['source_len']:i], 
                         (1,len(indices), model_meta['source_len']))
...
ax.set_xlim(min(obs['Date'].values),
            max(obs['Date'].values))
ax.set_ylim(min(min(prediction),min(target)-model_meta['persistence_error']*2)-273.15-1,
            max(max(prediction),max(target)+model_meta['persistence_error']*2)-273.15+1)
ax.set_ylabel('2m Temperature (C)')
ax.plot(obs['Date'].values,
                (target+model_meta['persistence_error']*2)-273.15,
                markersize=0,
                color='grey',
                marker='.',
                linewidth=0.5,
...
ax2.set_ylim(min(min(delta_p),min(delta_m))-2,
...
fig.savefig('Predictions_air.2m.png')
